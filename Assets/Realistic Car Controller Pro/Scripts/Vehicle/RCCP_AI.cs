...
public class RCCP_AI : RCCP_Component {
    ...
    public RCCP_AIWaypointsContainer waypointsContainer;                 // Waypoints container to be used to follow.
    ...
    public LayerMask obstacleLayers = -1;       //  Raycasts will detect these objects layers.
    public GameObject obstacle;     //  Current raycasted obstacle.

    public bool useRaycasts = true;     //	Using forward and sideways raycasts to avoid obstacles at runtime dynamically.
    internal float rayInput = 0f;                // Total ray input affected by the raycast distances.
    internal bool raycasting = false;        // Raycasts hits an obstacle now?
    internal float resetTime = 0f;           // This timer was used for deciding go back or not, after crashing.
    public bool reversingNow = false;       //  Currently reversing?
