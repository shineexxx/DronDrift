...
public class RCCP_Installation {
    ...
    public static void Check() {
        ...
        if (!layer_RCC)
            missingLayers[0] = "RCCP_Vehicle";

        if (!layer_RCC_WheelCollider)
            missingLayers[1] = "RCCP_WheelCollider";

        if (!layer_RCC_DetachablePart)
            missingLayers[2] = "RCCP_DetachablePart";

        if (!layer_RCC || !layer_RCC_DetachablePart || !layer_RCC_WheelCollider) {
            ...
            if (EditorUtility.DisplayDialog("Found Missing Layers For Realistic Car Controller Pro", "These layers will be added to the Tags and Layers\n\n" + missingLayers[0] + "\n" + missingLayers[1] + "\n" + missingLayers[2], "Add")) {
                ...
                CheckLayer("RCCP_Vehicle");
                CheckLayer("RCCP_WheelCollider");
                CheckLayer("RCCP_DetachablePart");
            ...
            foreach (RCCP_CarController vehicle in RCCP_DemoVehicles.Instance.vehicles) {
                ...
                if (vehicle != null) {
                    ...
                    if (RCCP_Settings.Instance.RCCPWheelColliderLayer != null) {
                        ...
                        foreach (WheelCollider wc in vehicle.GetComponentsInChildren<WheelCollider>())
                    ...
                    if (RCCP_Settings.Instance.RCCPDetachablePartLayer != null) {
                        ...
                        foreach (RCCP_DetachablePart part in vehicle.GetComponentsInChildren<RCCP_DetachablePart>())
